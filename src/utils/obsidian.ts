...
export interface ObsidianSearchPayload {
  el?: HTMLElement;
  onStart?: Function;
...
export function obsidianSearch(payload: ObsidianSearchPayload) {
  const {
    el = document.body,
    onStart,
    onStop,
  ...
  // @ts-ignore
  const search: any = app.internalPlugins.plugins["global-search"].views.search(
    // @ts-ignore
